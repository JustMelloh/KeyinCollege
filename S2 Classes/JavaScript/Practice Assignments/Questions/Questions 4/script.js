...
//   let helloElements = str.filter(function (s) {
//     return /hello/i.test(s);
//   });

//   helloElements.forEach(function (element) {
//     console.log(element);
//   });
// }
...
//   var replaceTen = str.replace(/10/g, "Ten");
//   return replaceTen;
    
// }

// var result = replaceTen(strWith10);
// console.log(result);
 ...
 *
 ******************************************************************************/
// var clean = 'This is a sentence.  This is another. Some people say that this is "fun." Oh look! a (red  ) dog arrived!'
...
//  value = value.replace(/\(\s+/g, '(');
//  value = value.replace(/\s+\)/g, ')');
//  value = value.replace(/"fun."/g, "'fun'");
//  return value;
 
// }
// var cleaned = cleanUp(clean)
// console.log(cleaned);
...
function fixPostalCode(postalCode) {
  ...
  // Define a regular expression to match a valid postal code format
  var postalCodeRegex = /^[ABCEGHJKLMNPRSTVXY]\d[A-Z] ?\d[A-Z]\d$/;

  // Check if the postal code matches the valid format
  if (!postalCodeRegex.test(postalCode)) {
    throw new Error('Invalid postal code');
  }

  // Reformat the postal code as A1A 1A1
...
try {
  var formattedPostalCode = fixPostalCode('a1a 1a1');
  console.log(formattedPostalCode); // Should print 'A1A 1A1'
} catch (error) {
  console.error(error.message); // Will print 'Invalid postal code' for invalid input
}
...
function toProvince(postalCode, useLongForm = false) {
  try {
    // Use the fixPostalCode function to format and validate the postal code
    const formattedPostalCode = fixPostalCode(postalCode);
    
    //  mapping of first letters
    ...
    // Get the first letter(s) 
    const firstLetter = formattedPostalCode.charAt(0);
    
    // Check if the first letter is in the mapping
    if (provinceMapping.hasOwnProperty(firstLetter)) {
      // Return the province name based on the useLongForm argument
      return useLongForm ? getProvinceLongForm(provinceMapping[firstLetter]) : provinceMapping[firstLetter];
    } else {
      return null; // Invalid first letter, return null
    }
  } catch (error) {
    return null; // Invalid postal code, return null
  }
}


function getProvinceLongForm(shortForm) {
  const provinceLongForms = {
    'ON': 'Ontario',
    'QC': 'Quebec',
    'NS': 'Nova Scotia',
    'NB': 'New Brunswick',
  ...
  return provinceLongForms[shortForm] || shortForm; // Return long form or short form if not found
}
...
function checkFirstChar(value) {
  ...
  return isUppercase ? "String's first character is uppercase" : "String's first character is not uppercase"
